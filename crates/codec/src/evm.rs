...
impl<E: ByteOrder> Encoder<E, Bytes> for Bytes {
...
impl<E: ByteOrder, const N: usize> Encoder<E, FixedBytes<N>> for FixedBytes<N> {
...
impl Encoder<LittleEndian, Address> for Address {
    const HEADER_SIZE: usize = <Address>::len_bytes();
    fn encode<W: WritableBuffer<LittleEndian>>(&self, encoder: &mut W, field_offset: usize) {
        self.0.encode(encoder, field_offset)
    }
    fn decode_header(
        decoder: &mut BufferDecoder<LittleEndian>,
        field_offset: usize,
        result: &mut Address,
    ) -> (usize, usize) {
        <FixedBytes<{ Self::HEADER_SIZE }> as Encoder<
...
impl<E: ByteOrder, const BITS: usize, const LIMBS: usize> Encoder<E, Uint<BITS, LIMBS>>
    for Uint<BITS, LIMBS>
{
