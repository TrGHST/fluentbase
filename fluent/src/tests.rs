...
//     let wasm_binary = include_bytes!("../../runtime/examples/bin/evm_verify_block_rlps.wasm"); //
// TODO     let rwasm_binary = wasm2rwasm(wasm_binary);
...
// from_str::<block::Block>(block_receipt_a_json.as_str()).unwrap();     let block_a =
// block_init.clone();     println!("TRANSACTIONS_BEFORE: {:?}", block_a.transactions);
...
// from_str::<receipt::Receipt>(receipt_json.as_str()).unwrap();
...
fn test_mpt_transactions_root() {
    ...
    // read block.json
    let mut block_receipt_a_json = String::new();
    File::open("src/test_data/block_with_transactions_b.json")
        .unwrap()
        .read_to_string(&mut block_receipt_a_json)
        .unwrap();
    ...
    println!("TRANSACTIONS_BEFORE: {:?}", block.transactions_root());
